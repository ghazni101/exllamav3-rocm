...
)
{
    ...
    auto _k = quant_cache_cont_kernel_instances[bits - 2];
    _k<<<num_blocks, MAX_WARPS * 32, 0, stream>>>
    (
...
)
{
    ...
    int num_blocks = CEIL_DIVIDE(bsz, MAX_WARPS * 4);
    auto _k = dequant_cache_cont_kernel_instances[bits - 2];
    _k<<<num_blocks, MAX_WARPS * 32, 0, stream>>>
    (
...
)
{
    ...
    auto _k = dequant_cache_paged_kernel_instances[k_bits - 2][v_bits - 2];
    _k<<<blocks, threads, 0, stream>>>
    (
